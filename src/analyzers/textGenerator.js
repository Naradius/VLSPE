...
import JAPEGenerator from "./japeGenerator.js";
...
export default class TextGenerator {
    constructor(elements, type) {
        ...
        switch (type) {
            case "jape":
                this.processor = new JAPEGenerator(file);
                break;
        }
    }
    ...
    processRule(ruleEl) {
        this.file += this.processor.preProcessRule(ruleEl);
        if (ruleEl.sequence.length > 0) {
            const seqNext = ruleEl.sequence[0];
            this.processAnRepAl(seqNext);
        }
        this.file += this.processor.postProcessRule(ruleEl);
    }
    ...
    processAnnotation(annotEl) {
        ...
        this.file = fileBcp;
        if (fileAdd.length > 1) {
            this.file += this.processor.preAnnotationGroup(fileAdd);
        }
        this.file += this.processor.annotationGroup(fileAdd);;
        if (fileAdd.length > 1) {
            this.file += this.processor.postAnnotationGroup(fileAdd);
        }
